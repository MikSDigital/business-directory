...
class JsonRpcController implements ContainerAwareInterface
{
        ...
        if (is_callable(array($service, $method))) {
            ...
            foreach ($params as $index => $param) {
                ...
                if (is_array($param) && !$rps[$index]->isArray() && $rps[$index]->getClass() != null) {
                    ...
                    $deserializationContext->setSerializeNull(true);

                    if (isset($action)) {
                        $deserializationContext->setGroups([$action]);
                    }
                    $params[$index] = $this->container->get('jms_serializer')
                        ->deserialize($param, $class, 'json', $deserializationContext);
                }
            }
            ...
            if ($this->container->has('jms_serializer')) {
                ...
                if (isset($action)) {
                    $serializationContext->setGroups([$action]);
                }

                $response = $this->container->get('jms_serializer')
                    ->serialize($response, 'json', $serializationContext);
            } else {
     ...
     * @return string
     */
    protected function convertExceptionToErrorData(\Exception $e, $prefixMessage = null)
    {
        return $prefixMessage ? $prefixMessage . '. ' . $e->getMessage() : $e->getMessage();
    }
